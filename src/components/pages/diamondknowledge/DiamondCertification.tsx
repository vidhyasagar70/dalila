...
export default function DiamondCertification() {
  const steps = [
    {
      number: '1.',
      title: 'Decide your budget and prioritize the 4Cs based on what matters most to you.'
    },
    {
      number: '2.',
    ...
    {
      number: '4.',
      title: 'Buy from a trusted jeweler with transparent sourcing and quality assurance.'
    }
  ];
          ...
          {steps.map((step, index) => (
            <div key={index}>
              <div className="flex items-start gap-1 mb-2">
                <span className={`text-5xl md:text-6xl lg:text-[4rem] font-normal text-gray-900 leading-none ${playFair.className}`}>
                  {step.number}
                </span>
              </div>
              <p className={`text-gray-800 leading-relaxed text-base md:text-lg max-w-5xl mx-auto font-light mb-3 ${playFair.className}`}>
